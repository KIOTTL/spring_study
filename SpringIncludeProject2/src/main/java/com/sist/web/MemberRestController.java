...
public class MemberRestController {
	...
	public String member_ajax_login_ok(String id, String pwd, HttpSession session) {
		String result="";
		MemberVO vo=dao.memberLogin(id, pwd);
		result=vo.getMsg();
	...
	// text/plain (json) , text/html (일반 문자열, script)
	public String member_vue_login_ok(String id, String pwd, HttpSession session) {
		String result="";
		MemberVO vo=dao.memberLogin(id, pwd);
		if(vo.getMsg().equals("OK")) {
			session.setAttribute("id", id);
		...
		JSONObject obj=new JSONObject();	// result:{} 객체단위

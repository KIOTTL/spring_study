package com.sist.web;

import org.springframework.context.ApplicationContext;
import org.springframework.context.support.ClassPathXmlApplicationContext;

/*
 * 	컴포넌트 : 한 개의 기능 (일부) , 게시판, 공지사항, ...
 * 	컨테이너 : 컴포넌트 여러개를 모아서 관리
 * 	DI => 클래스와 클래스의 연관 관계도
 * 	==> 컨테이너 (ApplicationContext : 컴포넌트를 생성하고 관리)
 * 
 * 	1. DI : Basic
 * 	2. AOP : Transaction / log / security
 * 	3. MVC : DI+AOP
 */

public class MainClass {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		// 1. 컨테이너에 XML파일 전송 ==> XML 파싱 후 메모리 할당 ==> setter에 값 주입
		/*
		 * 	컨테이너 => 클래스
		 * 	BeanFactory : Core(DI)
		 * 		|
		 * 	ApplicationContext	ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ   :	Core+AOP
		 * 		|									    |
		 * 	WebApplicationContext		AnnotationConfigApplicationContext(5버전)
		 * 	  Core+AOP+MVC						Core+AOP+Annotation
		 * 
		 */
		
		ApplicationContext app=new ClassPathXmlApplicationContext("app.xml");
		SawonVO sa1=(SawonVO)app.getBean("sa1");	// 등록된 id를 가지고 해당 객체 찾아오기 ==> DL
		sa1.print();
		
		SawonVO sa2=app.getBean("sa2", SawonVO.class);
		sa2.print();
		
		SawonVO sa3=(SawonVO)app.getBean("sa3");
		sa3.print();
		// 메모리 할당 => HashMap에 저장 (객체의 주소가 동일 => 싱글턴)
		// DI (값 주입 ==> database, 서버)
	}

}

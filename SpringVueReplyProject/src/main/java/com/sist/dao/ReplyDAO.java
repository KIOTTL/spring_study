...
public class ReplyDAO {
	...
	public List<ReplyVO> replyListData(ReplyVO vo) {
		...
		try {
			...
			cs.registerOutParameter(3, OracleTypes.CURSOR);	// 무조건 cursor 아님
	...
	public void replyInsert(ReplyVO vo) {
		try {
			...
			cs.executeQuery();	// 무조건 executeQuery (항상 select이므로)
			// <select ~~ statement="CALLABLE">{CALL replyInsert(?,?,?,?,?)}</select>	=> 항상 select
		} catch(Exception ex) {
			ex.printStackTrace();
		} finally {
			disConnection();

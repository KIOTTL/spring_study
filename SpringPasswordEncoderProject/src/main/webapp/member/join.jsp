    ...
    $('#idBtn').on("click",function(){
    	...
    	*/
    	$.ajax({
    		...
    		data:{"id":id},		// params
    		success:function(result){		// then
    			let res=result.trim();
    			// alert(res);
    			if(res==='YES'){
    				let msg='<span style="color: blue">'+id+'는(은) 사용가능한 아이디입니다</span>';
    				$('#result').html(msg)
    				$('#okid').show()
    			} else {
    				let msg='<span style="color: red">'+id+'는(은) 사용 중인 아이디입니다</span>';
    				$('#result').html(msg)
    			}
    		}
    	})
      ...
      <table class="table">
        <tr>
          ...
          <td width=90%>
            ...
            <input type=button class="btn btn-sm btn-danger" value="아이디 중복 체크" id="idcheck">
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td width=90%>
            <input type=text name="tel1" size=3 class="input-sm" value="010" readonly="readonly">-
